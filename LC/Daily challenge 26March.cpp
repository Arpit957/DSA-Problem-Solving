...
public:
  void dfs(int node,vector<int>&edges,vector<bool>&vis,vector<bool>&extra,vector<int>&dist,int &ans,int distance){
    ...
    int longestCycle(vector<int>& edges) {
        ...
        //We take another vector extra to suppose there is no cycle but the visited vector is already true it detects a cycle but there is no cycle. So for the cycle, both the vectors to be true at the node.
